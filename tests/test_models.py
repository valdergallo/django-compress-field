...
UPLOADED_FILE_FULL_PATH = os.path.join(
    settings.MEDIA_ROOT, 'mycontent/2014/06/03/', 'test_fixture.txt')
UPLOADED_ZIPFILE_FULL_PATH = os.path.join(
    settings.MEDIA_ROOT, 'mycontent/2014/06/03/', 'test_fixture.zip')

UPLOADED_FILE_PATH = os.path.join('mycontent/2014/06/03/', 'test_fixture.txt')
UPLOADED_ZIPFILE_PATH = os.path.join(
    'mycontent/2014/06/03/', 'test_fixture.zip')
UPLOADED_ZIPFILE2_PATH = os.path.join(
    'mycontent/2014/06/03/', 'test_fixture_1.zip')
...
class TestCompressTestCase(TestCase):
    ...
    def test_compress_name(self):
        my_content = self.create_my_content()
        self.assertEqual(my_content.upload_file.compress_name,
                         UPLOADED_ZIPFILE_PATH)
    ...
    def test_if_zip_is_not_override_names(self):
        ...
        self.assertNotEqual(my_content2.upload_file.name.replace(
            '\\', '/'), UPLOADED_ZIPFILE_PATH)
